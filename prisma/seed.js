...
function generateTribeDefenders(level, isGold = false) {
  ...
  // Units based on level
  const units = {};
  if (level === 1) {
    units.warrior = randInt(5, 15);
    units.archer = randInt(3, 8);
  } else if (level === 2) {
    units.warrior = randInt(15, 30);
    units.archer = randInt(10, 20);
    units.cavalry = randInt(3, 8);
  } else {
    units.warrior = randInt(30, 50);
    units.archer = randInt(20, 35);
    units.cavalry = randInt(10, 20);
    units.elite = randInt(3, 8);
  }
